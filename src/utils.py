...
def get_email_details(json_file_path=None):
    ...
    try:
        ...
        if not all(key in email_details for key in ['sender', 'receiver', 'subject', 'body']):
            raise KeyError(f"The JSON {file_path} must contain 'sender', 'receiver', 'subject' and 'body' keys.")
...
def format_excel(csv, column_name):
    # df = pd.read_csv(csv)
    df = pd.DataFrame(csv)
